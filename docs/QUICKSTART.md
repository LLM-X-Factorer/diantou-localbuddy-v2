...
> 适用版本：当前公开但未签名的 Engineering Alpha Release `v0.12.6 / Private Run Storage + Product Truth`。当前灰度与发布优先 Windows；macOS 保留回归，Linux 降为维护。开始前先阅读 [`KNOWN-LIMITATIONS.md`](KNOWN-LIMITATIONS.md)。
...
| Windows x64 | 使用公开 [`v0.12.6` Release](https://github.com/LLM-X-Factorer/diantou-localbuddy-v2/releases/tag/v0.12.6) 的 Setup/ZIP | Release Runner 覆盖合同、无凭据首启、安装版合成灰度、上一稳定版原地升级/profile 保留和五项资产校验；终端用户 Windows 11、代码签名和真实 OTA 仍待验收 |
