...
```text
选择一个 Run → 用户填写最小现象/复现 → 生成公开安全预览
→ 本机检查公开 Issue 是否同签名 → 用户确认 → 系统浏览器打开 Issue Form
```
...
现象和复现步骤还会遮盖常见 API Key/Bearer Token/凭据字段、带凭据 URL、macOS/Linux/Windows 用户路径、邮箱、`@` 提及和 HTML 标记。这只是最后一道机械保护，不能替代人工预览。

## 同意与防漂移合同

1. Renderer 不直接访问 GitHub；Main 进程只读取公开 Issue 列表并在本机匹配签名，不发送报告内容，也不带 Authorization；同一签名短时复用内存结果，避免预览与打开各消耗一次匿名请求；
2. 所有将被预填的公开字段和本机数据边界说明必须先显示，用户明确勾选公开确认后才能继续；
3. Main 进程重新生成报告，并核对预览 SHA-256。Run 状态或文字在预览后变化时 fail closed，要求重新预览；
...
- 确定性测试证明敏感 Run 字段和用户输入中的常见 secret/path 不进入预览或 URL；
- 最大中文输入仍低于保守的 7,500 字符预填 URL 上限；
- 重复 Issue、GitHub 离线/限流、恶意外链、预览后状态变化和本地保存均有测试；
- `pnpm check`、Renderer/Core build 和生产依赖高危审计通过；
- Electron 开发构建目视检查输入、预览、隐私确认和按钮禁用状态；
