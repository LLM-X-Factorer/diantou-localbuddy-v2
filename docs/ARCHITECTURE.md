...
21. **v0.12.2 Windows Canary + Safe Updates（已发布）/ v0.12.3 Public Update Bridge（候选）**：可追踪构建身份、按 SHA 并存 Canary、上一稳定版 Squirrel 原地升级门禁和 Run/Integration 空闲重启 Gate。仓库已按 Apache-2.0 公开，`0.12.3` stable 候选已内置公开 GitHub feed；桥接 Release、代码签名和 Windows 11 真人 OTA 仍开放。
