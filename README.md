...
- [`docs/KNOWN-LIMITATIONS.md`](docs/KNOWN-LIMITATIONS.md)：`v0.12.8` 分发基线与仍开放门禁；
...
- [`docs/M13-PRODUCT-TRUTH-SPRINT.md`](docs/M13-PRODUCT-TRUTH-SPRINT.md)：固定 `v0.12.8`、Research Desk 真实运行矩阵、非作者用户门和 `advance/pause/stop` 裁决；
...
- [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md)：当前 `v0.12.8` 架构事实；
...
- [`docs/WINDOWS-UPDATE-VALIDATION.md`](docs/WINDOWS-UPDATE-VALIDATION.md)：`v0.12.4-v0.12.8` 更新桥接、第一方公网升级与 Windows 11 开放门禁；
...
当前主动暂缓的是正式 Apple Developer ID、生产 Hardened Runtime entitlements、notarization 和公开 Gatekeeper 验收。`v0.12.8` 的 Windows Tag 门禁覆盖安装级无凭据首启、完整安装版合成灰度、从上一稳定版原地升级、profile 保留、五项分发资产校验和第一方公网 Squirrel 真升级。终端用户 Windows 11、真实 Provider 和第三方生产 MCP OAuth 仍需外部验收，不能用托管 Runner、公开 endpoint 或本地夹具冒充。
