...
| 分发 | `v0.13.1` 是当前 Windows-first 公开但未签名的 Engineering Alpha；五项资产、stable 灰度、本地/公网升级和独立回下载已通过 |
| 当前阶段 | M13 Product Truth Sprint；固定 `v0.13.1` Release 作为下一轮产品事实基线，真实 Provider、非作者用户和 Windows 11 证据重新计数 |
...
- [`docs/M13-PRODUCT-TRUTH-SPRINT.md`](docs/M13-PRODUCT-TRUTH-SPRINT.md)：固定 `v0.13.1` Release、Research Desk 真实运行矩阵、非作者用户门和 `advance/pause/stop` 裁决；
- [Issue #39](https://github.com/LLM-X-Factorer/diantou-localbuddy-v2/issues/39)：从点头真实内部工作出发的 Agent 化分诊与影子运行漏斗；
...
- [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md)：当前 `v0.13.1` 架构事实与历史已发布能力；
...
当前主动暂缓的是正式 Apple Developer ID、生产 Hardened Runtime entitlements、notarization 和公开 Gatekeeper 验收。`v0.13.1` 已完成 Windows Tag 门禁、安装级无凭据首启、完整安装版合成灰度、`v0.12.8 -> v0.13.1` 原位升级、profile 保留、五项分发资产校验和第一方公网 Squirrel 真升级。终端用户 Windows 11、真实 Provider 和第三方生产 MCP OAuth 仍需外部验收，不能用托管 Runner、公开 endpoint 或本地夹具冒充。
