...
- 首次使用只推荐一个熟悉、低风险的真实 Job：把一份完全虚构或用户明确选择的会议记录整理成可编辑的 `会议纪要.docx`。准备示例不调用模型；确认计划前不读取资料；原文件不会被修改；缺失信息必须标记为“待确认”。
- 完整工作台从持久审计事件投影出“当前进展、用户可理解步骤、已验证结果和下一步”；模型调用、工具事件、耗时及并行关系保留在默认收起的详细时间线中，不再让 Agent 图和 Token 指标抢占主路径。
- 本地 Skill 和 MCP 以“方法与连接”目录呈现，只发现 `.localbuddy/skills` 与 `.localbuddy/mcp.json` 中的受限元数据。发现不等于启用，用户必须明确选择；副作用调用仍逐次审批，不自动连接、下载或安装扩展。
...
- `README`、Quickstart、Roadmap、Known Limitations、Dogfood 和 M13 固定到同一 `v0.13.0` 候选口径；旧版本发布证据保留，不因候选发布回写历史。
...
- `0.13.0` macOS arm64 App、ZIP 和 DMG 已实际生成；ad-hoc 签名、DMG 完整性、隔离无凭据首启和真实 Electron 首次任务界面通过，方法与连接目录在隔离工作区中通过且没有尝试真实 MCP 连接；
...
- 固定 Tag、stable 安装版灰度、`v0.12.8 -> v0.13.0` 本地/公网升级、五项正式资产和独立回下载仍属于发布门禁，未完成前不得写成已发布。
